...
import { ETHTABLE, ETHEREUM } from "./config";
...
// put data
(async () => {
  // const result = await dataHandler.putDataWithEms({year: 2021, month: 5, date:19}, {year:2021, month:5, date:20}, ETHEREUM);
  const result = await dataHandler.putTenData({year: 2020, month: 10, date:17}, 'alpha');
